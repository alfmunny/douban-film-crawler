...
也有不容易被禁的方法。为了加快爬取的效率，会对爬虫并行化，集群化，分布式爬取。然后通过维护一个代理IP池，让访问随机的来自不同的IP。并且可以加入一些频率上的随机扰动或者schedule来让爬虫不那么刻意。 再通过调节一个合理的访问频率，可以比较稳定的爬取，凡事都要互相体谅。
...
1. [爬虫博客归档](https://xlzd.me/2017/11/21/crawler-archive/)